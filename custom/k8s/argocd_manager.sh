...
# Pick an app name with fzf from `argocd app list`. $1 = prompt.
pick_app() {
  local prompt="${1:-Select app: }"
  if ! command -v fzf >/dev/null 2>&1; then
    echo "fzf not installed; pass the app name explicitly." >&2
    exit 1
  fi
  local choice
  choice=$(argocd app list -o name 2>/dev/null \
    | fzf --height 40% --reverse --prompt="$prompt" --exact || true)
  [[ -z "$choice" ]] && { echo "No app selected."; exit 0; }
  echo "$choice"
}
