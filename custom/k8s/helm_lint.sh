...
# Pick the chart dir: from the first positional, or fzf over Chart.yaml owners.
if [[ $# -ge 1 ]]; then
  CHART="$1"
elif command -v fzf >/dev/null 2>&1; then
  echo "Searching for charts (Chart.yaml) under $(pwd)..."
  chart_file=$(find . -maxdepth 4 -type f -name Chart.yaml -print0 \
    | xargs -0 -n1 dirname 2>/dev/null \
    | fzf --height 40% --reverse --prompt="Select chart: " --exact || true)
  [[ -z "$chart_file" ]] && { echo "No chart selected."; exit 0; }
  CHART="$chart_file"
else
