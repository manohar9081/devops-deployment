...
if [[ -z "$POD" ]]; then
  ...
  echo "Tip: pick a pod interactively with:"
  echo "  k_doctor.sh \$(kubectl get pods -A -o jsonpath='{range .items[*]}{.metadata.namespace}{\"\\t\"}{.metadata.name}{\"\\n\"}{end}' | fzf | awk '{print \$1\" \"\$2}')"
  exit 1
fi
...
echo "Started   : $(kubectl get pod "${NS_FLAG[@]}" "$POD" \
  -o jsonpath='{.status.startTime}' 2>/dev/null || echo '?')"
echo "Phase     : $(kubectl get pod "${NS_FLAG[@]}" "$POD" \
  -o jsonpath='{.status.phase}' 2>/dev/null || echo '?')"
echo "Restarts  : $(kubectl get pod "${NS_FLAG[@]}" "$POD" \
  -o jsonpath='{.status.containerStatuses[0].restartCount}' 2>/dev/null || echo '?')"
echo "Last term : $(kubectl get pod "${NS_FLAG[@]}" "$POD" \
  -o jsonpath='{.status.containerStatuses[0].lastState.terminated.reason}' 2>/dev/null || echo 'none')"
echo "Exit code : $(kubectl get pod "${NS_FLAG[@]}" "$POD" \
...
case "${SLICE:-full}" in
  logs)
    echo ">>> current logs:"
    kubectl logs "${NS_FLAG[@]}" "$POD" --tail=80 || true
    ;;
  prev)
    echo ">>> previous-container logs (the crash reason):"
    kubectl logs "${NS_FLAG[@]}" "$POD" --previous --tail=80 || true
    ;;
  *)
    echo ""
    echo ">>> Events for $POD:"
    kubectl describe pod "${NS_FLAG[@]}" "$POD" | sed -n '/^Events:/,$p' | head -40 || true
    echo ""
    echo ">>> Current logs (tail 40):"
    kubectl logs "${NS_FLAG[@]}" "$POD" --tail=40 || true
    echo ""
    echo ">>> Previous logs (tail 40):"
    kubectl logs "${NS_FLAG[@]}" "$POD" --previous --tail=40 2>/dev/null \
