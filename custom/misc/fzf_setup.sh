...
#   TAB          fuzzy-search aliases + functions + history + PATH executables
#                (only fires on an empty command line; normal TAB elsewhere)
...
# `install-fzf-setup.sh` installer (menu option 8) to enable it, or call
...
# ---- TAB: combined aliases/functions/history/executables picker ----
__fzf_tab_command() {
  ...
  cmds=$(compgen -c 2>/dev/null | sort -u || true)
  {
    ...
    | fzf --height 40% --reverse --exact --no-sort \
          ...
          --header "TAB mode | type to filter | Enter to insert" \
    || true
}

__fzf_tab_widget() {
  local selected
  selected=$(__fzf_tab_command)
...
if [[ -n "${BASH_VERSION:-}" ]]; then
  bind -x '"\t": __fzf_tab_widget'      2>/dev/null || true
...
if [[ "${BASH_SOURCE[0]}" == "${0}" ]]; then
  echo "fzf keybindings active. TAB=commands, Ctrl+R=history, Ctrl+T=files, Alt+C=cd."
fi
