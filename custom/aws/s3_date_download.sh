...
SOURCE_BUCKET=$(echo "$BUCKETS" | fzf --height 40% --reverse --prompt="Select source bucket:" --exact || true)
[[ -z "$SOURCE_BUCKET" ]] && die "No source bucket selected."
...
SELECTED=$(echo "$MATCHES" | fzf --height 40% --reverse \
