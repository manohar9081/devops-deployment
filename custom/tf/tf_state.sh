...
case "$action" in
  ...
  browse|"")
    if ! command -v fzf >/dev/null 2>&1; then
      echo "fzf not installed — use: $0 list|find|show|count"; exit 1
    fi
    picked=$(run_list | fzf --height 40% --reverse \
      --prompt="Pick a resource (ENTER to show, ESC to quit): " --exact || true)
