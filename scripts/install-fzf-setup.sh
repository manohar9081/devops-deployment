...
strip_block() {
  local tmp
  [[ ! -f "$BASHRC" ]] && return 0
  # sed with line-range delete between the markers (inclusive).
  tmp=$(mktemp)
  sed "/^${MARK_BEGIN}\$/,/^${MARK_END}\$/d" "$BASHRC" > "$tmp" || true
...
build_block() {
  ...
  echo ""
  echo "$MARK_BEGIN"
  echo "# Added by install-fzf-setup.sh. Edit or remove via menu option 8."
  case "$choice" in
    fzf)
...
if [[ $# -ge 1 ]]; then
  ...
  echo "  1  - fzf keybindings only   (TAB / Ctrl+R / Ctrl+T / Alt+C)"
...
if [[ "$MODE" == "fzf" || "$MODE" == "both" ]]; then
  if ! command -v fzf >/dev/null 2>&1; then
    ...
    echo "Run 'install-bashtools' (menu option 7) first, then re-run this."
    exit 1
  fi
fi
