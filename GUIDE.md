...
│   ├── install-fzf-setup.sh   ← opt-in fzf/cursor setup (menu option 8)
│   └── add_creds_to_aws.sh
│
...
│   │   ├── s3_local_objects.sh
│   │   └── sg_audit.sh
│   ├── k8s/               ← Kubernetes helpers (10 scripts)
...
You need **all five** of these. If any is missing, the install will tell you.
...
> Optional but recommended: **`fzf`** (fuzzy finder — powers the nice interactive
> menus). It is installed automatically by `install-bashtools.sh`
...
Select an option:
  ...
  7  - Install Helium Browser
  8  - Install Bash tools
  9  - Setup fzf/cursor (opt-in)
  10 - Install all
  11 - Install all (except fzf/cursor, Helium and Brave Browser)
Enter option:
```

> Note: option **9 is opt-in**. "Install all" (10/11) does **not** enable the
> fzf keybindings or cursor styling — choose 9 separately only if you want them.

**Recommended for a first install:** type `11` and press Enter. This installs
everything except the fzf/cursor, Helium and Brave browser. When it finishes:
...
one-line change. If you type an invalid option it now exits cleanly instead of
...
Installs **fzf** (the fuzzy finder that powers the interactive menus) and then
...
=== fzf / cursor setup (opt-in) ===
  1  - fzf keybindings only   (TAB / Ctrl+R / Ctrl+T / Alt+C)
...
##### `ec2_connect.sh`
Browse running EC2 instances (Name, state, IP) via fzf and start an SSM
...
##### `argocd_manager.sh`
ArgoCD application manager. A dispatcher over the `argocd` CLI with fzf
...
--render`, or just `helm_lint.sh` to fzf-pick a chart.
...
**TAB** to a combined fuzzy-search menu (aliases + functions + history +
...
into the selection. (Alias: `fzf_setup`.) **Disabled by default** — enable it
permanently via menu option 8 (`install-fzf-setup.sh`), or run `fzf_setup` for
...
permanently via menu option 8, or run `set_cursor` for just the current terminal.
...
fzf), `k8s_version`, `tf_version` (switch kubectl / terraform version),
`sso_login`, `sso_profile`, `ssm_session`, `aws_profile`, and the prompt
helpers `kubectl_prompt_info` / `git_branch_info`.
